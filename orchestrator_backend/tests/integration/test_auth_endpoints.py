...
def test_auth_login_admin_success(django_user_model):
...
@pytest.mark.django_db
def test_auth_login_employee_success(django_user_model):
...
@pytest.mark.django_db
def test_auth_login_role_mismatch(django_user_model):
    employee_group, _ = Group.objects.get_or_create(name=ROLE_GROUP_MAP[EMPLOYEE_ROLE])
    user = django_user_model.objects.create_user(username='emp2', password='pass1234')
    user.groups.add(employee_group)

    client = APIClient()
    resp = client.post('/api/auth/login/admin', {'username': 'emp2', 'password': 'pass1234'}, format='json')
